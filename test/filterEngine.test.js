...
test('combined board attaches Filter Tips separately from All Picks and VAR Tips',()=>{
...
test('straight win publishes a favourite priced 1.20 to 1.55',()=>{
...
test('one pick per match tries markets in listed order',()=>{
  const result=diagnoseFilterFixture(fixture({
    marketOdds:markets({
...
test('early season, similar form and cup ties are skipped',()=>{
  assert.equal(diagnoseFilterFixture(fixture({earlySeason:true})).skip,'early-season')
  const similar=diagnoseFilterFixture(fixture({
...
test('direction disagreement skips that market then tries the next',()=>{
  const result=diagnoseFilterFixture(fixture({
