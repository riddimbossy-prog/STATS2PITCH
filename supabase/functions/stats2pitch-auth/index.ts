...
Deno.serve(async req=>{
  ...
  if(route==='/health'&&req.method==='GET')return json({ok:true,service:'stats2pitch-auth',version:'2.3.0'})
  ...
  if(route==='/signup'){
    ...
    if(SUPABASE_SERVICE_ROLE_KEY){
      ...
      if(!created.ok){
        ...
        if(/already|registered|exists/i.test(message))return json({error:'That email already has an account. Sign in instead.'},400)
