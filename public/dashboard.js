...
import {shell,renderBoard,showRefresh,renderLive,stopLive,setTabActive} from './boardView.js?v=2.2.4'
...
export async function startDashboard(){document.body.classList.remove('auth-page');document.body.classList.add('board-page');state.date=new URLSearchParams(location.search).get('date')||localToday();shell({openFilters:()=>openFilters({loadBoard,renderBoard}),openProfile,manualRefresh,switchTab});await loadBoard()}
