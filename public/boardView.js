...
const availabilityLabel=status=>({qualified:'Pick ready','waiting-odds':'Check back soon','waiting-verified-odds':'Check back soon','insufficient-split':'Check back soon',priced:'No pick yet','analysis-unavailable':'Check back soon','analysis-ready':'Check back soon',scheduled:'Scheduled'})[status]||'Scheduled'
...
export function renderBoard(){
  ...
  const rows=state.tab==='best'?best:three,title=state.tab==='best'?'Best Picks':'Top Rated'
  document.getElementById('section-title').textContent=title;document.getElementById('section-sub').textContent=rows.length?`${rows.length} selection${rows.length===1?'':'s'} · earliest kickoff first`:rawHasQualified?'0 selections match your choices':'No picks available';host.innerHTML=rows.length?rows.map(card).join(''):rawHasQualified?`<div class="empty">No picks match your current choices. Try another market or pick strength.</div>`:noTipsMarkup();host.querySelectorAll('[data-details]').forEach(b=>b.onclick=()=>openDetails(b.dataset.details,b.dataset.market))
}
