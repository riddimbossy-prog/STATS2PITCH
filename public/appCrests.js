...
function card(r,i){const score=scoreFor(r),x=resultFor(r),stage=seasonStageFor(r),banker=bankerApproved(r);const conf=Number.isFinite(Number(r.engineRating))?Math.round(Number(r.engineRating)):Math.round(Math.min(Number(r.homeConsensus)||0,Number(r.awayConsensus)||0));const confPct=Math.max(0,Math.min(100,conf));const ev=expectedValue(r.odds,confPct);const evStr=ev==null?'':`${ev>=0?'+':''}${ev.toFixed(2)}`;const confHtml=conf>0?`<div class="confidence"><span>${Number.isFinite(Number(r.engineRating))?'Engine score':'Confidence'}</span><div class="confidence-bar"><i style="width:${confPct}%"></i></div><b>${conf}${Number.isFinite(Number(r.engineRating))?'':'%'}</b></div>`:'';return`<article class="card ${stateFor(r)} ${stage==='early'?'early-season-card':stage==='solid'?'solid-season-card':''}" data-i="${i}" role="button" tabindex="0" aria-label="Why ${esc(r.home)} vs ${esc(r.away)} was chosen"><div class="m-card-top">${banker?`<span class="m-banker"><span class="m-star">★</span> BANKER</span>`:`<span class="m-kick-row">${esc(kickClock(r.kickoff))}</span>`}${conf>0?`<span class="m-conf">Confidence ${esc(conf)}%</span>`:''}</div>${banker?`<div class="m-kick-row">${esc(kickClock(r.kickoff))}</div>`:''}${leagueLine(r)}${statusBadge(r)}${banker?'<span class="banker-badge">BANKER</span>':''}${matchup(r,score)}${score&&stateFor(r)==='live'?`<div class="live-score-note">${esc(x?.minute||'')}′</div>`:''}<div class="pick"><div class="pick-copy"><span class="pick-kicker">PICK</span><strong>${esc(marketLabel(r))}</strong></div><span class="odd">${Number(r.odds).toFixed(2)}</span></div>${confHtml}<div class="m-footer">${ev!=null?`<div class="m-ev"><span>Expected Value</span><b class="${ev>=0?'pos':'neg'}">${evStr}</b></div>`:'<div class="m-ev"></div>'}<span class="odd odd-stack"><span class="odd-kicker">ODDS</span>${Number(r.odds).toFixed(2)}</span></div><div class="time"><b>Kickoff</b> · ${formatDateTime(r.kickoff)}</div>${earlyNote(r)}<button class="details" data-i="${i}" type="button">Why this pick?</button><div class="m-why-row">Why this pick ›</div></article>`}
