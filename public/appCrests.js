...
function resultFor(r){const live=state.resultData?.picks?.find(x=>String(x.fixtureId)===String(r.fixtureId))?.result||null,stored=state.board?.results?.[String(r.fixtureId)]||null;if(live&&decided(live.outcome))return live;if(live&&live.matchState==='live')return live;if(stored&&decided(stored.outcome))return stored;return live||stored}
