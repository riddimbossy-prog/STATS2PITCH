...
function matchMid(r,score){const live=stateFor(r)==='live';if(score)return`<span class="versus">${live?'LIVE':'VS'}</span><b class="match-mid-score">${esc(score.home)}–${esc(score.away)}</b>`;return`<span class="versus">VS</span><b class="match-mid-clock">${esc(kickClock(r.kickoff))}</b>`}
...
function boardReady(board){const meta=board?.varTipsMeta||{},engine=String(meta.engine||board?.meta?.varTipsEngine||'');return engine===REQUIRED_ENGINE&&Array.isArray(board?.varTips)}
function resultFor(r){return(state.results?.varTips||[]).find(x=>String(x.fixtureId)===String(r.fixtureId))?.result||null}
...
function allRows(){return boardReady(state.board)?(state.board.varTips||[]).filter(r=>!isSrlPick(r)):[]}
...
function renderStats(rows){const host=$('#varStats');if(!host)return;const counts={upcoming:0,live:0,settled:0,total:rows.length};for(const r of rows){const s=stateFor(r);if(counts[s]!==undefined)counts[s]++}host.innerHTML=`<button data-stat="all"><small>VAR Tips</small><b>${counts.total}</b></button><button data-stat="upcoming"><small>Upcoming</small><b>${counts.upcoming}</b></button><button data-stat="live"><small>Live</small><b>${counts.live}</b></button><button data-stat="settled"><small>Settled</small><b>${counts.settled}</b></button>`;$$('[data-stat]').forEach(b=>b.onclick=()=>{state.status=b.dataset.stat;if($('#statusFilter'))$('#statusFilter').value=state.status;render()})}
function card(r,i){const score=scoreFor(r),fav=String(r.favourite||'')==='home'?'HOME FAV':String(r.favourite||'')==='away'?'AWAY FAV':'',odds=oddStr(r);return`<article class="card ${stateFor(r)}" data-i="${i}" role="button" tabindex="0" aria-label="Why ${esc(r.home)} vs ${esc(r.away)} was chosen"><div class="m-card-top"><span class="m-board-tag">VAR</span>${fav?`<span class="m-fav-tag ${esc(r.favourite)}">${fav}</span>`:''}<span class="m-kick-row">${esc(kickClock(r.kickoff))}</span></div><div class="league"><span class="league-flag" role="img" aria-label="${esc(r.country||'International')} flag">${flag(r.country)}</span><span>${esc(r.league||'League')}</span></div>${statusBadge(r)}<span class="var-badge">VAR</span>${fav?`<span class="fav-badge ${esc(r.favourite)}">${fav}</span>`:''}${matchup(r,score)}<div class="pick"><div class="pick-copy"><span class="pick-kicker">PICK</span><strong>${esc(pickLabel(r))}</strong></div><span class="odd">${odds}</span></div><div class="m-footer"><div class="m-ev"><span>${esc(marketName(r))}</span></div><span class="odd odd-stack"><span class="odd-kicker">ODDS</span>${odds}</span></div><div class="time"><b>${esc(marketName(r))}</b> · ${formatDateTime(r.kickoff)}</div><button class="details" data-i="${i}" type="button">Why this pick?</button><div class="m-why-row">Why this pick ›</div></article>`}
...
function render(){
  ...
  if(!ready){
    ...
    $('#cards').innerHTML='<div class="empty">VAR Tips will appear here after the next board refresh.</div>'
    return
  }
...
function pickRows(board){return (board?.varTips||[]).filter(r=>!isSrlPick(r))}
