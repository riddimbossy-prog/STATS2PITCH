...
function team(name,img){return`<div class="banker-rule-team"><span><img class="team-crest" src="${esc(img)}" alt="${esc(name)} crest" loading="lazy"></span><b>${esc(name)}</b></div>`}
...
function card(r,fx){const h=r?.metrics?.home||{},a=r?.metrics?.away||{},l=r?.metrics?.league||{};return`<article class="banker-rule-card">
  ...
  <div class="banker-rule-match">${team(r.home,crestSrc(r,'home',fx))}<strong>VS</strong>${team(r.away,crestSrc(r,'away',fx))}</div>
...
function render(){
  ...
  $('#bankerCards').innerHTML=rows.length?rows.map(r=>card(r,fx)).join(''):'<div class="empty">No fixture passed the dedicated Banker rules for this date.</div>'
  bindCrestFallbacks($('#bankerCards'))
}
...
$('#ruleFilter').onchange=e=>{state.rule=e.target.value;render()};$('#countryFilter').onchange=e=>{state.country=e.target.value;state.league='all';render()};$('#leagueFilter').onchange=e=>{state.league=e.target.value;render()};$('#refresh').onclick=load;if($('#notifyBell'))$('#notifyBell').onclick=load;if($('#profileBtn'))$('#profileBtn').onclick=()=>document.body.classList.toggle('filters-open')
load()
