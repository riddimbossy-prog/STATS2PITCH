  ...
  root.innerHTML = `
    <main class="login-shell">
      <section class="login-card">
        ...
        <p class="login-note">${creating ? 'Instant account access is enabled when Supabase Email “Confirm email” is turned OFF.' : 'The prediction board is protected. You must be signed in to access it.'}</p>
  ...
  form.onsubmit = async e => {
    ...
    try {
      if (creating) {
        ...
        // If a project still requires confirmation, try a password login so the
        // owner gets a direct configuration message instead of a silent failure.
        if (!created?.access_token) {
          try {
            await signIn(email, password)
          } catch {
            throw new Error('Account created, but Supabase is still requiring email verification. In Supabase → Authentication → Providers → Email, turn “Confirm email” OFF.')
          }
        }
      } else {
        await signIn(email, password)
