...
function canAddAccaLeg(slip,pick){
  ...
  if(isComboPick(pick))return{ok:false,reason:'combo-not-on-slip'}
  if(!PUBLISHED_ROUTES.has(pick.route))return{ok:false,reason:'no-pick'}
...
function slipCopy(code){return({
  ...
  'combo-not-on-slip':'Combo covers stay off the slip.',
