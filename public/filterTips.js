...
function pricedOk(r){const n=Number(r.odds);return Number.isFinite(n)&&n>=1.2&&n<=2.2}
function allRows(){return boardReady(state.board)?(state.board.filterTips||[]).filter(r=>!isSrlPick(r)&&pricedOk(r)):[]}
...
function pickRows(board){return (board?.filterTips||[]).filter(r=>!isSrlPick(r)&&pricedOk(r))}
