...
export function confirmLast5(f,published,favourite){
  ...
  const homeAll=venueRows(f?.home?.formHistory||f?.home?.fixtures||f?.home?.lastMatches,homeId,'home')
  const awayAll=venueRows(f?.away?.formHistory||f?.away?.fixtures||f?.away?.lastMatches,awayId,'away')
  const sides=[
  ...
  for(const row of sides){
    ...
    const avg=hitRate(row.all.slice(0,BANKER_RULES.formAvgSample),row.venue,row.test)
    ...
    if(finite(avg.rate)&&avg.played>sample&&avg.rate<avgMin)return{ok:false,skip:'form-avg',reasons:[`${row.label} average ${formLabel(published)} is ${avg.rate}% over ${avg.played} ${row.venue} games — below ${avgMin}%. Last 5 does not confirm the average.`]}
    if(ofAvg!=null&&ofAvg<avgMin)return{ok:false,skip:'form-avg',reasons:[`${row.label} last ${sample} is ${last.rate}% vs average ${avg.rate}% (${ofAvg}% of average) — below ${avgMin}%.`]}
    ...
    detail.push({side:row.side,label:row.label,venue:row.venue,last,avg,ofAvg})
  }
  return{ok:true,skip:null,reasons,detail}
}
