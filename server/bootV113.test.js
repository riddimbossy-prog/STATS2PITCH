...
assert(index.includes("dataset.s2pUiBuild='1.13.1'"),'index must expose current UI build 1.13.1')
assert(index.includes('boot-resilience-v1.13.0.js'),'deterministic bootstrap must stay loaded')
assert(!index.includes('bootstrap-shell-v1.12.3.js'),'competing shell watchdog must stay out of production startup')
assert(index.includes('board-loading-surface-v1.13.0.js'),'visible loading surface must stay loaded')
...
assert(cacheReset.includes("const BUILD='1.13.1'"),'cache reset must run once for the current UI build')
