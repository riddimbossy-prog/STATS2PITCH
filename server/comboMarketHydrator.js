...
export function keepEventMarket(m){
  ...
  const n=norm(m?.name||m?.desc||'')
  if(String(m?.name||m?.desc||'').includes('&')||/corner|booking|card|throw in|offside|player|penalt|first half|1st half|second half|2nd half|half time|minute/i.test(n))return false
