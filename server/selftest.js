...
import { leagueGamesPlayed, leagueTotalCompletedGames, hasMinimumLeagueGames, MIN_LEAGUE_GAMES, SPLIT_ENGINE_POLICY } from './stats.js'
import { filterMatureFixtures, snapshotHasStrictMaturityPolicy, snapshotHasStrictSplitPolicy, EARLY_SEASON_POLICY } from './maturity.js'
...
const oneTeamStillThree=[{team:{id:1},all:{played:4}},{team:{id:2},all:{played:4}},{team:{id:3},all:{played:3}},{team:{id:4},all:{played:4}}]
const allTeamsFour=[{team:{id:1},all:{played:4}},{team:{id:2},all:{played:4}},{team:{id:3},all:{played:4}},{team:{id:4},all:{played:4}}]
...
const standingsByLeague=new Map([[1,largeEarlyLeague],[2,largeEarlyLeague],[3,largeEarlyLeague],[4,allTeamsFour],[5,allTeamsFour]])
const preflight=await selectMatureCandidates(preflightFixtures,async league=>standingsByLeague.get(league)||[],2)
if(preflight.selected.length!==2||preflight.selected[0].f.fixture.id!==201||preflight.selected[1].f.fixture.id!==202) throw new Error('Maturity-first candidate selection failed')
...
if(parsed.over15!==1.18||parsed.over25!==1.72||parsed.under35!==1.47) throw new Error('Nested total-goals parsing failed')
...
const strongHome={id:1,name:'Alpha',venue:'home',position:1,overallPosition:4,leagueSize:12,played:2,overallPlayed:4,ppg:2.5,goalsScored:2.4,goalsConceded:.7,winRate:80,lossRate:0,winRate10:null,lossRate10:null,formAgreement:'INSUFFICIENT',over15:80,under15:20,over25:60,under25:40,over35:20,under35:80,cleanSheetRate:60,failedToScoreRate:0,bttsRate:40}
const weakAway={id:2,name:'Beta',venue:'away',position:12,overallPosition:8,leagueSize:12,played:2,overallPlayed:4,ppg:.5,goalsScored:.8,goalsConceded:2.5,winRate:20,lossRate:80,winRate10:null,lossRate10:null,formAgreement:'INSUFFICIENT',over15:80,under15:20,over25:60,under25:40,over35:20,under35:80,cleanSheetRate:20,failedToScoreRate:60,bttsRate:40}
const fixture={fixtureId:1,match:'Alpha vs Beta',league:'Test',country:'Test',kickoff:new Date().toISOString(),kickoffLocal:'Today',marketOdds,odds:{...parsed},leagueMinimumPlayed:4,earlySeasonEligible:true,splitPolicy:SPLIT_ENGINE_POLICY,home:strongHome,away:weakAway}
if(filterMatureFixtures([fixture]).length!==1) throw new Error('Mature fixture failed overall-games second gate')
const immatureFixture={...fixture,fixtureId:99,leagueMinimumPlayed:3,home:{...strongHome,overallPlayed:3}}
if(filterMatureFixtures([immatureFixture]).length!==0) throw new Error('Immature fixture passed second gate')

const board=buildBoard(filterMatureFixtures([fixture]),{fixturesScanned:1,generatedAt:new Date().toISOString(),earlySeasonPolicy:EARLY_SEASON_POLICY,minimumLeagueGames:MIN_LEAGUE_GAMES,splitPolicy:SPLIT_ENGINE_POLICY,splitPrimaryOnly:true})
...
if(!snapshotHasStrictMaturityPolicy(board)) throw new Error('Fresh board missing four-game maturity marker')
if(!snapshotHasStrictSplitPolicy(board)) throw new Error('Fresh board missing strict split marker')
if(snapshotHasStrictSplitPolicy({...board,meta:{...board.meta,splitPolicy:'legacy-overall'}})) throw new Error('Legacy overall snapshot was accepted as split-safe')
...
const unpricedBoard=buildBoard(filterMatureFixtures([unpricedFixture]),{splitPolicy:SPLIT_ENGINE_POLICY,splitPrimaryOnly:true})
if(unpricedBoard.priority.length!==0) throw new Error('Unpriced fixture was incorrectly published')

console.log(JSON.stringify({ok:true,minimumOverallLeagueGames:MIN_LEAGUE_GAMES,splitPolicy:SPLIT_ENGINE_POLICY,earlySkipped:preflight.earlySeasonSkipped,pricingPolicy:board.meta.pricingPolicy,filters:board.groups.threePlus[0].filterCount},null,2))
