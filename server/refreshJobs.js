...
export function createRefreshJobs(runRefresh,{ttlMs=30*60*1000,staleMs=5*60*1000,store=null}={}){
  ...
  async function persist(job,force=false){if(!job.persistent||!store?.save)return false;const now=Date.now();if(!force&&job._lastPersist&&now-job._lastPersist<1500)return true;job._lastPersist=now;return store.save(job).catch(()=>false)}
  ...
  async function start(date){
    ...
    if(store?.claim){const claim=await store.claim(key,instance,Math.ceil(staleMs/1000)).catch(()=>({supported:false,claimed:true}));persistent=claim?.supported===true;if(persistent&&!claim.claimed){const remote=await store.load?.(key).catch(()=>null);if(remote)return publicJob(remote)}}
    const stamp=nowIso(),job={date:key,status:'running',ownerId:instance,startedAt:stamp,updatedAt:stamp,finishedAt:null,progress:{phase:'queued',message:'Refresh queued.',current:0,total:null},error:null,board:null,persistent}
