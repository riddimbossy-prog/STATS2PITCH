...
const MAX_PAGES=Math.max(1,Number(process.env.STATS_API_FALLBACK_MATCH_PAGES||3))
...
const loose=(a,b)=>{const x=norm(a),y=norm(b);if(!x||!y)return false;if(x===y)return true;if(Math.min(x.length,y.length)>=5&&(x.includes(y)||y.includes(x)))return true;const xa=new Set(x.split(' ')),ya=new Set(y.split(' '));return [...xa].filter(t=>t.length>2&&ya.has(t)).length>=1}
...
async function searchVariant(params,home,away,kickoff){
  ...
  for(let page=1;page<=MAX_PAGES;page++){
    ...
    for(const m of rows){
      ...
      const direct=loose(h,home)&&loose(a,away)
      const reversed=loose(h,away)&&loose(a,home)
      if(direct||reversed)found.push({m,direct})
    }
    if(rows.length<100)break
  }
  if(!found.length)return null
  found.sort((x,y)=>(x.direct===y.direct?0:x.direct?-1:1)||dist(matchDate(x.m),kickoff)-dist(matchDate(y.m),kickoff))
  const best=found[0]?.m
  if(!best||dist(matchDate(best),kickoff)>48*3600*1000)return null
  return best
}
...
export async function getStatsOddsFallback(fixture){
  ...
  let match=null
  for(const params of variants){if(!date)break;match=await searchVariant(params,home,away,kickoff);if(match)break}
