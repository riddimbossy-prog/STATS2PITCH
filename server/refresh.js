...
import {buildBankerRules,buildLeagueScoringProfile} from './bankerEngine.js'
...
const jobs=new Map(),leagueCache=new Map(),teamCache=new Map(),splitCache=new Map()
...
export async function refreshNow(date,onProgress=()=>{}){
  ...
  const analyzed=await mapLimit(scheduled,Math.max(1,Number(process.env.REFRESH_CONCURRENCY||2)),async f=>{
    try{
      ...
      if(homeFixtures.length<FORM_SAMPLE||awayFixtures.length<FORM_SAMPLE){insufficientHistory++;done++;onProgress({stage:'analyzing',done,total:scheduled.length,statsVerified,fallbackTeams,insufficientHistory,analysisErrors});return null}
      ...
      done++;onProgress({stage:'analyzing',done,total:scheduled.length,statsVerified,fallbackTeams,insufficientHistory,analysisErrors})
      return{fixtureId:f.fixture.id,league:f.league?.name||'',country:f.league?.country||'',kickoff:f.fixture.date,home:{id:homeId,name:f.teams.home.name,logo:f.teams.home.logo||null,fixtures:homeFixtures},away:{id:awayId,name:f.teams.away.name,logo:f.teams.away.logo||null,fixtures:awayFixtures},earlySeason,earlySeasonHome,earlySeasonAway,currentVenueSamples:{home:currentHomeFixtures.length,away:currentAwayFixtures.length},over25Profile,bankerLeagueProfile,homeSplit,awaySplit,marketOdds}
    }catch(error){analysisErrors++;console.warn(`Fixture ${f?.fixture?.id||'unknown'} skipped: ${error?.message||error}`);done++;onProgress({stage:'analyzing',done,total:scheduled.length,statsVerified,fallbackTeams,insufficientHistory,analysisErrors});return null}
  })
  ...
  const board=buildBoard(tierEligible,{date,generatedAt:new Date().toISOString(),engineVersion:ENGINE_VERSION,sourceFixtures:raw.length,scheduledFixtures:scheduled.length,analyzedFixtures:fixtures.length,insufficientHistoryFixtures:insufficientHistory,analysisErrorFixtures:analysisErrors,tierEligibleFixtures:tierEligible.length,sameTierSkipped,tierUnverifiedSkipped,sameTierOrUnverifiedSkipped:sameTierSkipped+tierUnverifiedSkipped,over25ProfiledFixtures:fixtures.filter(f=>f.over25Profile).length,statsVerifiedFixtures:statsVerified,historyFallbackTeams:fallbackTeams,bankerRules:bankerRules.meta,diagnostics:{sourceFixtures:raw.length,scheduledFixtures:scheduled.length,insufficientHistoryFixtures:insufficientHistory,analysisErrorFixtures:analysisErrors,analyzedFixtures:fixtures.length,sameTierSkipped,tierUnverifiedSkipped,tierEligibleFixtures:tierEligible.length,qualifiedTips:0,bestPicks:0,bankerRulePicks:bankerRules.picks.length}},learned)
  ...
  const picks=new Map(board.bestPicks.map(p=>[String(p.fixtureId),p])),eligibleIds=new Set(tierEligible.map(f=>String(f.fixtureId)))
