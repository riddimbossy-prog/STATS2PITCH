...
import { buildBoard, TEAM_RESULT_ELIGIBILITY_POLICY, GG_POLICY } from './engine.js'
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(u.pathname==='/api/board'){
  ...
  if(board&&board?.meta?.winSafetyPolicy!==WIN_SAFETY_POLICY)return json(res,200,emptyMatureBoard(staleMeta({safetyReason:'Saved board predates the current Top-3-only result safety policy. Refresh real data.'})))
