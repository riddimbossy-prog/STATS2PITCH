...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(u.pathname==='/api/config')return json(res,200,{supabaseUrl:process.env.SUPABASE_URL||'',supabaseAnonKey:process.env.SUPABASE_ANON_KEY||'',allowPublicSignup:String(process.env.ALLOW_PUBLIC_SIGNUP||'true')==='true',enableGithubLogin:String(process.env.ENABLE_GITHUB_LOGIN||'true')==='true'})
