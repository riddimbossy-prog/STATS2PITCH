...
async function runRefreshCore(requestedDate,progress=()=>{}){
  ...
  const {date,fixtures,rawFixtures,rawCount,upcomingCount,maturityCandidates,maturityScanned,candidatesAttempted,attemptLimit,candidatePoolExhausted,enrichmentTargetReached,earlySeasonSkipped,standingsUnavailable,statsOddsFallbacks,statsFallbackCapSkipped,unpricedSkipped,shortSplitHistory,enrichmentErrors,freshProviderReads}=enriched
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(u.pathname==='/api/health')return json(res,200,{ok:true,brand:'Stats2Pitch.com',version:VERSION,splitPolicy:SPLIT_ENGINE_POLICY,engineIntegrityPolicy:ENGINE_INTEGRITY_POLICY,winSafetyPolicy:WIN_SAFETY_POLICY,refreshMode:'background-job',refreshPersistence:refreshStoreMode(),candidatePolicy:'full-maturity-discovery-priced-enrichment',oddsPolicy:'single-bookmaker-coherent-v1',rankingPolicy:'family-diversity-first-v1',contradictionPolicy:'high-veto-moderate-safer-only',time:new Date().toISOString()})
